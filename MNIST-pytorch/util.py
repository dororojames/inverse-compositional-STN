...
def imread(fname):
    return cv2.imread(fname)/255.0


def imsave(fname, array):
    cv2.toimage(array, cmin=0.0, cmax=1.0).save(fname)
...
def restoreModel(opt, geometric, classifier, it):
    geometric.load_state_dict(torch.load(
        "models_{0}/{1}_it{2}_GP.npy".format(opt.group, opt.model, it)))
    classifier.load_state_dict(torch.load(
        "models_{0}/{1}_it{2}_C.npy".format(opt.group, opt.model, it)))
# save model


def saveModel(opt, geometric, classifier, it):
    torch.save(geometric.state_dict(),
               "models_{0}/{1}_it{2}_GP.npy".format(opt.group, opt.model, it))
...
class Visdom():
    ...
    def tileImages(self, opt, images, H, W, HN, WN):
        ...
        imageBlocks = np.concatenate(
            [np.concatenate(row, axis=2) for row in images], axis=1)
        return imageBlocks
    ...
    def trainLoss(self, opt, it, loss):
        ...
        if self.trainLossInit:
            ...
            self.vis.line(Y=np.array([loss]), X=np.array(
                [it]), win=opt.model+"_trainloss", update="append")
    ...
    def testLoss(self, opt, it, loss):
        if self.testLossInit:
            ...
            self.vis.line(Y=np.array([loss]), X=np.array(
                [it]), win=opt.model+"_testloss", update="append")
    ...
    def meanVar(self, opt, mean, var):
        ...
        self.vis.image(mean[0].clip(0, 1), win="{0}_meaninit".format(
            opt.model), opts={"title": "{0} (TEST_mean_init)".format(opt.model)})
        self.vis.image(mean[1].clip(0, 1), win="{0}_meanwarped".format(
            opt.model), opts={"title": "{0} (TEST_mean_warped)".format(opt.model)})
        self.vis.image(var[0].clip(0, 1), win="{0}_varinit".format(opt.model), opts={
                       "title": "{0} (TEST_var_init)".format(opt.model)})
        self.vis.image(var[1].clip(0, 1), win="{0}_varwarped".format(opt.model), opts={
                       "title": "{0} (TEST_var_warped)".format(opt.model)})
