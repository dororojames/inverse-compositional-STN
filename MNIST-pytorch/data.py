...
def loadMNIST(opt, path):
    ...
    trainData, testData = {}, {}
    trainData["image"] = torch.FloatTensor(
        [np.array(sample[0])/255.0 for sample in trainDataset])
    testData["image"] = torch.FloatTensor(
        [np.array(sample[0])/255.0 for sample in testDataset])
...
def genPerturbations(opt):
    ...
    if opt.warpType == "homography":
        ...
        pPert = np.matmul(np.linalg.inv(np.matmul(Jtransp, J)),
                          np.matmul(Jtransp, dXY)).squeeze()
    pInit = torch.from_numpy(pPert).unsqueeze(0)
    return pInit
    ...
    for b in range(batchN):
        ...
        if opt.netType == "STN" or opt.netType == "IC-STN":
            ...
            for i in range(len(realIdx)):
                ...
                if l not in warped[0]:
                    warped[0][l] = []
                if l not in warped[1]:
                    warped[1][l] = []
