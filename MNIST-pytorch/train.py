...
if opt.fromIt != 0:
    ...
    print(util.toMagenta(
        "resuming from iteration {0}...".format(opt.fromIt)))
...
for i in range(opt.fromIt, opt.toIt):
    ...
    if (i+1) % 100 == 0:
        print("it. {0}/{1}  lr={3}(GP),{4}(C), loss={5}, time={2}"
              .format(util.toCyan("{0}".format(i+1)),
                      opt.toIt,
                      util.toGreen("{0:.2f}".format(
                          time.time()-timeStart)),
                      util.toYellow("{0:.0e}".format(lrGP)),
                      util.toYellow("{0:.0e}".format(lrC)),
                      util.toRed("{0:.4f}".format(train_loss))))
    if (i+1) % 200 == 0:
        vis.trainLoss(opt, i+1, train_loss)
    if (i+1) % 1000 == 0:
        # evaluate on test set
        testAcc, testMean, testVar = data.evalTest(
            opt, testData, geometric, classifier)
    ...
    if (i+1) % 10000 == 0:
        ...
        print(util.toGreen(
            "model saved: {0}/{1}, it.{2}".format(opt.group, opt.model, i+1)))
